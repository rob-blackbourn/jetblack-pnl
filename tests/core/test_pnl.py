...
def test_long_to_short_with_splits_best_price():
    ...
    # Buy 6 @ 100
    pnl = add_trade(pnl, Trade(6, 100), unmatched, matched)
    assert pnl.quantity == 6
    assert pnl.cost == -600
    assert pnl.realized == 0

    # Buy 6 @ 106
    pnl = add_trade(pnl, Trade(6, 106), unmatched, matched)
    assert pnl.quantity == 12
    assert pnl.cost == -1236
    assert pnl.realized == 0

    # Buy 6 @ 103
    pnl = add_trade(pnl, Trade(6, 103), unmatched, matched)
    assert pnl.quantity == 18
    assert pnl.cost == -1854
    assert pnl.realized == 0

    # Sell 9 @ 105
    pnl = add_trade(pnl, Trade(-9, 105), unmatched, matched)
    assert pnl.quantity == 9
    assert pnl.cost == -945
    assert pnl.realized == 36

    # Sell 9 @ 107
    pnl = add_trade(pnl, Trade(-9, 107), unmatched, matched)
    assert pnl.quantity == 0
    assert pnl.cost == 0
...
def test_long_to_short_fifo_with_profit():
    ...
    pnl = add_trade(pnl, Trade(1, 100), unmatched, matched)
    pnl = add_trade(pnl, Trade(-1, 102), unmatched, matched)
    assert pnl.quantity == 0
    assert pnl.cost == 0
...
def test_long_sell_fifo_through_flat():
    ...
    pnl = add_trade(pnl, Trade(1, 101), unmatched, matched)
    pnl = add_trade(pnl, Trade(-2, 102), unmatched, matched)
...
def test_short_buy_fifo_through_flat():
    ...
    pnl = add_trade(pnl, Trade(-1, 102), unmatched, matched)
    pnl = add_trade(pnl, Trade(2, 101), unmatched, matched)
    assert pnl.quantity == 1
    assert pnl.cost == -101
    assert pnl.realized == 1
    assert unmatched == UnmatchedPool.Fifo((
...
def test_one_buy_many_sells_fifo():
    ...
    pnl = add_trade(pnl, Trade(10, 101), unmatched, matched)
    pnl = add_trade(pnl, Trade(-5, 102), unmatched, matched)
    assert pnl.quantity == 5
    assert pnl.cost == -505
    assert pnl.realized == 5
    pnl = add_trade(pnl, Trade(-5, 104), unmatched, matched)
    assert pnl.quantity == 0
    assert pnl.cost == 0
...
def test_pnl():
    ...
    # Buy 10 @ 100
    pnl = add_trade(pnl, Trade(10, 100), unmatched, matched)
    ...
    pnl = add_trade(pnl, Trade(10, 102), unmatched, matched)
    ...
    pnl = add_trade(pnl, Trade(-10, 104), unmatched, matched)
...
def test_many_buys_one_sell_fifo():
    ...
    pnl = add_trade(pnl, Trade(1, 102), unmatched, matched)
    pnl = add_trade(pnl, Trade(1, 101), unmatched, matched)
    pnl = add_trade(pnl, Trade(1, 104), unmatched, matched)
    pnl = add_trade(pnl, Trade(1, 103), unmatched, matched)
    pnl = add_trade(pnl, Trade(-5, 104), unmatched, matched)
    assert matched == MatchedPool((
        (
            SplitTrade(Decimal(1), Trade(1, 100)),
...
def test_fraction_quantities():
    ...
    pnl = add_trade(
        pnl,
        Trade(Decimal("10.17"), Decimal("2.54")),
        unmatched, matched
    )
    pnl = add_trade(
        pnl,
        Trade(Decimal("-8.17"), Decimal("2.12")),
        unmatched, matched
    )
    assert pnl.quantity == 2
    pnl = add_trade(
        pnl,
        Trade(Decimal("-1.5"), Decimal("2.05")),
        unmatched, matched
    )
